...
#define FIFO_NAME "/home/deeplearn/Desktop/video_pcie_trans/pango_pcie_driver/pango_pcie_dma_alloc2/adc_fifo"
#define FIFO_NAME2 "/home/deeplearn/Desktop/video_pcie_trans/pango_pcie_driver/pango_pcie_dma_alloc2/eth_fifo"
#define VALID_DATA_SIZE 16384  // 每次传输的有效数据大小
...
SCREEN_INFO GuiScreen[] = 
{
	...
	/* Write DDR Button */
	{100, 	"Eth data",BUTTON,NOTEBOOK_FRAME_FIXED_INDEX_2,but_w,but_h,lx2,ly2+ls2*4+10,	TRUE,	write_ddr_button,"clicked",NULL},
	/* DMA Read Button */
	{101,	"Adc data",BUTTON,NOTEBOOK_FRAME_FIXED_INDEX_2,but_w,but_h,lx2,ly2+ls2*5+15,	TRUE,	dma_read_button,"clicked", NULL},
	/* Allocate Mem Entry */
	...
	/* DMA Write Button */
	{106,	"Config_ed",BUTTON,NOTEBOOK_FRAME_FIXED_INDEX_2,but_w,but_h,ex2,ey2+es2*4+15,	TRUE,	read_ddr_button,"clicked", NULL},
	/* Read DDR Button */
...
void video_display(){
	...
	{	
		while(idle_flag.flag_data == flag && receive_flag == true)
		{
			usleep(1000);
...
// WAV 文件头结构
void write_wav_header(FILE *wav_file, uint32_t sample_rate, uint16_t num_channels, uint32_t data_size) {
    ...
    fwrite(&chunk_size, sizeof(chunk_size), 1, wav_file);  // ChunkSize
    fwrite("WAVE", 1, 4, wav_file);  // Format
    fwrite("fmt ", 1, 4, wav_file);  // Subchunk1ID
    uint32_t fmt_chunk_size = 16;  // Subchunk1Size for PCM
    fwrite(&fmt_chunk_size, sizeof(fmt_chunk_size), 1, wav_file);
    uint16_t audio_format = 1;  // AudioFormat (PCM)
    fwrite(&audio_format, sizeof(audio_format), 1, wav_file);
    fwrite(&num_channels, sizeof(num_channels), 1, wav_file);  // NumChannels
    fwrite(&sample_rate, sizeof(sample_rate), 1, wav_file);  // SampleRate
    fwrite(&byte_rate, sizeof(byte_rate), 1, wav_file);  // ByteRate
    fwrite(&block_align, sizeof(block_align), 1, wav_file);  // BlockAlign
    fwrite(&bits_per_sample, sizeof(bits_per_sample), 1, wav_file);  // BitsPerSample
    fwrite("data", 1, 4, wav_file);  // Subchunk2ID
    fwrite(&data_size, sizeof(data_size), 1, wav_file);  // Subchunk2Size (size of data)
}

// 更新 WAV 头部信息
void update_wav_header(const char *filename, uint32_t data_size) {
    FILE *wav_file = fopen(filename, "r+b");
    if (!wav_file) {
        perror("Failed to open file to update header");
        return;
    }
    ...
    fwrite(&chunk_size, sizeof(chunk_size), 1, wav_file);
...
// 初始化 WAV 文件并写入初始头部
FILE* init_wav_file(const char *filename, uint32_t sample_rate, uint16_t num_channels) {
    FILE *wav_file = fopen(filename, "wb");
    if (!wav_file) {
        perror("Failed to open file");
        return NULL;
    }
    write_wav_header(wav_file, sample_rate, num_channels, 0); // 初始 data_size 为 0
    fclose(wav_file);
...
// 写入音频数据到 WAV 文件的函数，每次写入20000字节
void write_audio_data(FILE *wav_file, DMA_DATA_NEW *dma_data, int data_size) {
    if (wav_file) {
        fwrite(dma_data->write_buf, 1, data_size, wav_file);
        fflush(wav_file);
        printf("Written %d bytes to WAV file.\n", data_size);
    }
}
...
void adc_display(){
	...
	memset(&dma_data1_1.write_buf,6,DMA_MAX_PACKET_SIZE);
	
	if(idle_flag_1.flag_data == 0 || dma_data1_1.write_buf[0] == 0 )
			{
				printf_info("DMA地址映射失败\n");
			}
   	 // 初始化fifo
	FILE *fifo_fd = fopen(FIFO_NAME, "w"); // 打开 FIFO 以写入
	if (!fifo_fd ) {
		printf("open fifo\n");
		return 0;
	}	
	printf("初始化成功\n");
	fwrite(dma_data1_1.write_buf,1, VALID_DATA_SIZE, fifo_fd);
    	fflush(fifo_fd); // 确保数据被写入
    	printf("Written 16384 bytes to FIFO.\n");
	usleep(1000000/60*2);
	printf_info("开始\n");
	bool flag2 = false;
	while (receive_flag == true)
	{	
		...
		flag2 = true;
		while(idle_flag_1.flag_data == flag && receive_flag == true)
		{
			usleep(1000);
			ioctl(pci_driver_fd, PCI_READ1_DATA3, &idle_flag_1);	//往fpga发送dma写指令
		}
		flag = idle_flag_1.flag_data;
		printf_info("adc 0x%08x\n",idle_flag_1.flag_data);
		if (idle_flag_1.flag_data == 0)
		{
			ioctl(pci_driver_fd, PCI_READ1_DATA2, &dma_data1_1);		//往fpga发送dma写指令
		}
		else
		{
			ioctl(pci_driver_fd, PCI_READ1_DATA1, &dma_data1_1);		//往fpga发送dma写指令
		}

	}
	printf_info("已退出线程\n");
	fclose(fifo_fd); // 关闭 FIFO
...
gboolean dma_read_button( GtkWidget *Widget, gpointer Data )
{
	g_thread_create((GThreadFunc)adc_display,NULL,FALSE,NULL);
}
...
void eth_display(){
	...
	memset(&dma_data1_2.write_buf,6,DMA_MAX_PACKET_SIZE);
	
	if(idle_flag_2.flag_data == 0 || dma_data1_2.write_buf[0] == 0 )
			{
				printf_info("DMA地址映射失败\n");
			}
   	 // 初始化fifo
	FILE *fifo_fd2 = fopen(FIFO_NAME2, "w"); // 打开 FIFO 以写入
	if (!fifo_fd2 ) {
		printf("open fifo\n");
		return 0;
	}	
	printf("初始化成功\n");
	fwrite(dma_data1_2.write_buf,1, VALID_DATA_SIZE, fifo_fd2);
    	fflush(fifo_fd2); // 确保数据被写入
	...
	{	
		...
		flag2 = true;
		while(idle_flag_2.flag_data == flag && receive_flag == true)
		{
			usleep(1000);
			ioctl(pci_driver_fd, PCI_READ2_DATA3, &idle_flag_2);	//往fpga发送dma写指令
		}
		flag = idle_flag_2.flag_data;
		printf_info("eth 0x%08x\n",idle_flag_2.flag_data);
		if (idle_flag_2.flag_data == 0)
		{
			ioctl(pci_driver_fd, PCI_READ2_DATA2, &dma_data1_2);		//往fpga发送dma写指令
		}
		else
		{
			ioctl(pci_driver_fd, PCI_READ2_DATA1, &dma_data1_2);		//往fpga发送dma写指令
		}

	}
	printf_info("已退出线程\n");
	fclose(fifo_fd2); // 关闭 FIFO
...
gboolean write_ddr_button( GtkWidget *Widget, gpointer Data )
{
	g_thread_create((GThreadFunc)eth_display,NULL,FALSE,NULL);
}
