...
//adc
#define PCI_READ1_DATA1		_IOWR(TYPE, 13, int)							//读缓存区1数据指令
#define PCI_READ1_DATA2		_IOWR(TYPE, 14, int)							//读缓存区2数据指令
#define PCI_READ1_DATA3		_IOWR(TYPE, 15, int)							//读缓存区flag数据指令
//eth
#define PCI_READ2_DATA1		_IOWR(TYPE, 16, int)							//读缓存区1数据指令
#define PCI_READ2_DATA2		_IOWR(TYPE, 17, int)							//读缓存区2数据指令
#define PCI_READ2_DATA3		_IOWR(TYPE, 18, int)							//读缓存区flag数据指令
...
#define DMA_MAX_PACKET_SIZE   			4094304										/* DMA包最大值 */ // 4MB, 
...
DMA_DATA_NEW dma_data2;
DMA_DATA_NEW dma_data1_1;
DMA_DATA_NEW dma_data2_1;
DMA_DATA_NEW dma_data1_2;
DMA_DATA_NEW dma_data2_2;
...
typedef struct _DMA_ADDR_NEW_  //新增的DMA地址结构体
{
	dma_addr_t addr;
	void* data_buf;
	spinlock_t lock;
...
typedef struct _DMA_INFO_NEW_
{
	DMA_ADDR_NEW flag;
	DMA_ADDR_NEW addr1;
	DMA_ADDR_NEW addr2;
	DMA_ADDR_NEW flag_1;
	DMA_ADDR_NEW addr1_1;
	DMA_ADDR_NEW addr2_1;
	DMA_ADDR_NEW flag_2;
	DMA_ADDR_NEW addr1_2;
	DMA_ADDR_NEW addr2_2;
}DMA_INFO_NEW;

DMA_INFO_NEW dma_info_new;
