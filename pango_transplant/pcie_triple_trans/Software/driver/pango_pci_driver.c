...
static void set_dma_addr_new(DMA_ADDR_NEW* dma_addr, struct PciPango *pci_pango, unsigned int offset)
{
	iowrite32(dma_addr->addr & 0x00000000ffffffff, pci_pango->_pango_pci_driver._pci_io + offset); //将DMA地址低32位写入到FPGA的DMA地址寄存器
	// if(dma_addr->addr_size)
	// 	iowrite32(dma_addr->addr >> 32, pci_pango->_pango_pci_driver._pci_io + RW_ADDR_HI_OFFSET);//如果DMA地址高32位不为0，则将DMA地址高32位写入到FPGA的DMA地址寄存器
}

static void set_dma_addr_high(DMA_ADDR_NEW* dma_addr, struct PciPango *pci_pango, unsigned int offset)
{
	//iowrite32(dma_addr->addr & 0x00000000ffffffff, pci_pango->_pango_pci_driver._pci_io + offset); //将DMA地址低32位写入到FPGA的DMA地址寄存器
	//if(dma_addr->addr_size)
	iowrite32(dma_addr->addr >> 32, pci_pango->_pango_pci_driver._pci_io + offset);//如果DMA地址高32位不为0，则将DMA地址高32位写入到FPGA的DMA地址寄存器
}

static void set_dma_addr(DMA_ADDR* dma_addr, struct PciPango *pci_pango)
{
	iowrite32(dma_addr->addr & 0x00000000ffffffff, pci_pango->_pango_pci_driver._pci_io + RW_ADDR_LO_OFFSET); //将DMA地址低32位写入到FPGA的DMA地址寄存器
	// if(dma_addr->addr_size)
	// 	iowrite32(dma_addr->addr >> 32, pci_pango->_pango_pci_driver._pci_io + RW_ADDR_HI_OFFSET);//如果DMA地址高32位不为0，则将DMA地址高32位写入到FPGA的DMA地址寄存器
}
...
long pango_cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case PCI_MAP_ADDR_CMD:
			...
			// 设置 DMA 地址掩码为 32 位,简化 DMA 在fpga中的地址的处理
            //if (dma_set_mask(&op_dev->dev, DMA_BIT_MASK(32)) || dma_set_coherent_mask(&op_dev->dev, DMA_BIT_MASK(32))) {
              //  dev_err(&op_dev->dev, "32-bit DMA configuration failed\n");
             //   printk("32-bit DMA configuration failed");
           // }
			dma_info_new.addr1.data_buf = pci_alloc_consistent(op_dev, DMA_MAX_PACKET_SIZE, &dma_info_new.addr1.addr); //为帧1缓存分配连续DMA地址，大小为DMA_MAX_PACKET_SIZE
			dma_info_new.addr2.data_buf = pci_alloc_consistent(op_dev, DMA_MAX_PACKET_SIZE, &dma_info_new.addr2.addr); //为帧2缓存分配连续DMA地址，大小为DMA_MAX_PACKET_SIZE
			dma_info_new.flag.data_buf  = pci_alloc_consistent(op_dev, 4, &dma_info_new.flag.addr);                    //为标志位缓存分配连续DMA地址，大小为4
			
			dma_info_new.addr1_1.data_buf = pci_alloc_consistent(op_dev, DMA_MAX_PACKET_SIZE, &dma_info_new.addr1_1.addr); //为帧1缓存分配连续DMA地址，大小为DMA_MAX_PACKET_SIZE
			dma_info_new.addr2_1.data_buf = pci_alloc_consistent(op_dev, DMA_MAX_PACKET_SIZE, &dma_info_new.addr2_1.addr); //为帧2缓存分配连续DMA地址，大小为DMA_MAX_PACKET_SIZE
			dma_info_new.flag_1.data_buf  = pci_alloc_consistent(op_dev, 4, &dma_info_new.flag_1.addr);                    //为标志位缓存分配连续DMA地址，大小为4
			
			dma_info_new.addr1_2.data_buf = pci_alloc_consistent(op_dev, DMA_MAX_PACKET_SIZE, &dma_info_new.addr1_2.addr); //为帧1缓存分配连续DMA地址，大小为DMA_MAX_PACKET_SIZE
			dma_info_new.addr2_2.data_buf = pci_alloc_consistent(op_dev, DMA_MAX_PACKET_SIZE, &dma_info_new.addr2_2.addr); //为帧2缓存分配连续DMA地址，大小为DMA_MAX_PACKET_SIZE
			dma_info_new.flag_2.data_buf  = pci_alloc_consistent(op_dev, 4, &dma_info_new.flag_2.addr);                    //为标志位缓存分配连续DMA地址，大小为4
			//dma_info_new.addr1.data_buf = pci_alloc_consistent(&op_dev->dev, DMA_MAX_PACKET_SIZE, &dma_info_new.addr1.addr,GFP_KERNEL); //为帧1缓存分配连续DMA地址，大小为DMA_MAX_PACKET_SIZE
			//dma_info_new.addr2.data_buf = pci_alloc_consistent(&op_dev->dev, DMA_MAX_PACKET_SIZE, &dma_info_new.addr2.addr,GFP_KERNEL); //为帧2缓存分配连续DMA地址，大小为DMA_MAX_PACKET_SIZE
			//dma_info_new.flag.data_buf  = pci_alloc_consistent(&op_dev->dev, 4, &dma_info_new.flag.addr,GFP_KERNEL);                    //为标志位缓存分配连续DMA地址，大小为4

			printk("dma_addr1  = 0x%llx\n", dma_info_new.addr1.addr);
			printk("dma_addr2  = 0x%llx\n", dma_info_new.addr2.addr);
			printk("dma_flag   = 0x%llx\n", dma_info_new.flag.addr);
			printk("dma_addr1_1  = 0x%llx\n", dma_info_new.addr1_1.addr);
			printk("dma_addr2_1  = 0x%llx\n", dma_info_new.addr2_1.addr);
			printk("dma_flag_1   = 0x%llx\n", dma_info_new.flag_1.addr);
			printk("dma_addr1_2  = 0x%llx\n", dma_info_new.addr1_2.addr);
			printk("dma_addr2_2  = 0x%llx\n", dma_info_new.addr2_2.addr);
			printk("dma_flag_2   = 0x%llx\n", dma_info_new.flag_2.addr);
			set_dma_addr_new(&dma_info_new.addr1, pci_pango,0x100);  //将DMA地址发送给FPGA设备，FPGA设备接收到这个地址后，会根据这个地址进行相应的操作
			set_dma_addr_new(&dma_info_new.addr1_1, pci_pango,0x110); 
			set_dma_addr_new(&dma_info_new.addr2, pci_pango,0x120);
			set_dma_addr_new(&dma_info_new.addr2_1, pci_pango,0x130);
			set_dma_addr_new(&dma_info_new.flag, pci_pango,0x140);
			set_dma_addr_new(&dma_info_new.flag_1, pci_pango,0x150);
			set_dma_addr_high(&dma_info_new.flag, pci_pango,0x160);
			set_dma_addr_new(&dma_info_new.addr1_2, pci_pango,0x170); 
			set_dma_addr_new(&dma_info_new.addr2_2, pci_pango,0x180); 
			set_dma_addr_new(&dma_info_new.flag_2, pci_pango,0x190);
			printk("发送成功");
			//初始化
			memset(&idle_flag.flag_data,1,4);
			memset(&dma_data1.write_buf,1,DMA_MAX_PACKET_SIZE);
			memset(&dma_data2.write_buf,1,DMA_MAX_PACKET_SIZE);
			memset(&idle_flag_1.flag_data,1,4);
			memset(&dma_data1_1.write_buf,1,DMA_MAX_PACKET_SIZE);
			memset(&dma_data2_1.write_buf,1,DMA_MAX_PACKET_SIZE);
			memset(&idle_flag_2.flag_data,1,4);
			memset(&dma_data1_2.write_buf,1,DMA_MAX_PACKET_SIZE);
			memset(&dma_data2_2.write_buf,1,DMA_MAX_PACKET_SIZE);
			//判断是否初始化成功
			if(dma_info_new.addr1.data_buf == NULL || dma_info_new.addr2.data_buf == NULL || dma_info_new.flag.data_buf == NULL)
			{
				printk("DMA地址映射失败\n");
			}

			//判断idle_flag等是否初始化成功
			if(idle_flag.flag_data == 0 || dma_data1.write_buf == 0 || dma_data2.write_buf == 0)
			{
				printk("DMA数据初始化失败\n");
			}
			else
		...
		case PCI_READ_DATA1:
			spin_lock(&dma_info_new.addr1.lock);
			//memset(dma_info_new.addr1.data_buf, 7, DMA_MAX_PACKET_SIZE);
			memcpy(dma_data1.write_buf, dma_info_new.addr1.data_buf, DMA_MAX_PACKET_SIZE);//将dma_info_new.addr1.data_buf中的数据写入到dma_data1.write_buf中  
			spin_unlock(&dma_info_new.addr1.lock);
			copy_to_user((DMA_DATA_NEW*)arg, &dma_data1, sizeof(DMA_DATA_NEW));
			
		break;

		case PCI_READ_DATA2:
			spin_lock(&dma_info_new.addr2.lock);
			//memset(dma_info_new.addr2.data_buf, 6, DMA_MAX_PACKET_SIZE);
			memcpy(dma_data2.write_buf, dma_info_new.addr2.data_buf, DMA_MAX_PACKET_SIZE);//将dma_info_new.addr2.data_buf中的数据写入到dma_data2.write_buf中
			spin_unlock(&dma_info_new.addr2.lock);
			copy_to_user((DMA_DATA_NEW*)arg, &dma_data2, sizeof(DMA_DATA_NEW));
			//spin_unlock(&dma_info_new.addr2.lock);
		break;

		case PCI_READ_DATA3:
		        //printk("进入成功");
			spin_lock(&dma_info_new.flag.lock);
			//printk("发送成功1");
			memcpy(&idle_flag.flag_data, dma_info_new.flag.data_buf, 4);
			spin_unlock(&dma_info_new.flag.lock);
			//printk("发送成功2");
			copy_to_user((IDLE_FLAG*)arg, &idle_flag, sizeof(IDLE_FLAG));
			//spin_unlock(&dma_info_new.flag.lock);
			//printk("成功");
		break;
		
		case PCI_READ1_DATA1:
			spin_lock(&dma_info_new.addr1_1.lock);
			//memset(dma_info_new.addr1.data_buf, 7, DMA_MAX_PACKET_SIZE);
			memcpy(dma_data1_1.write_buf, dma_info_new.addr1_1.data_buf, DMA_MAX_PACKET_SIZE);//将dma_info_new.addr1.data_buf中的数据写入到dma_data1.write_buf中  
			spin_unlock(&dma_info_new.addr1_1.lock);
			copy_to_user((DMA_DATA_NEW*)arg, &dma_data1_1, sizeof(DMA_DATA_NEW));
			
		break;

		case PCI_READ1_DATA2:
			spin_lock(&dma_info_new.addr2_1.lock);
			//memset(dma_info_new.addr2.data_buf, 6, DMA_MAX_PACKET_SIZE);
			memcpy(dma_data2_1.write_buf, dma_info_new.addr2_1.data_buf, DMA_MAX_PACKET_SIZE);//将dma_info_new.addr2.data_buf中的数据写入到dma_data2.write_buf中
			spin_unlock(&dma_info_new.addr2_1.lock);
			copy_to_user((DMA_DATA_NEW*)arg, &dma_data2_1, sizeof(DMA_DATA_NEW));
			//spin_unlock(&dma_info_new.addr2.lock);
		break;

		case PCI_READ1_DATA3:
		        //printk("进入成功");
			spin_lock(&dma_info_new.flag_1.lock);
			//printk("发送成功1");
			memcpy(&idle_flag_1.flag_data, dma_info_new.flag_1.data_buf, 4);
			spin_unlock(&dma_info_new.flag_1.lock);
			//printk("发送成功2");
			copy_to_user((IDLE_FLAG*)arg, &idle_flag_1, sizeof(IDLE_FLAG));
			//spin_unlock(&dma_info_new.flag.lock);
			//printk("成功");
		break;
		
		case PCI_READ2_DATA1:
			spin_lock(&dma_info_new.addr1_2.lock);
			//memset(dma_info_new.addr1.data_buf, 7, DMA_MAX_PACKET_SIZE);
			memcpy(dma_data1_2.write_buf, dma_info_new.addr1_2.data_buf, DMA_MAX_PACKET_SIZE);//将dma_info_new.addr1.data_buf中的数据写入到dma_data1.write_buf中  
			spin_unlock(&dma_info_new.addr1_2.lock);
			copy_to_user((DMA_DATA_NEW*)arg, &dma_data1_2, sizeof(DMA_DATA_NEW));
			
		break;

		case PCI_READ2_DATA2:
			spin_lock(&dma_info_new.addr2_2.lock);
			//memset(dma_info_new.addr2.data_buf, 6, DMA_MAX_PACKET_SIZE);
			memcpy(dma_data2_2.write_buf, dma_info_new.addr2_2.data_buf, DMA_MAX_PACKET_SIZE);//将dma_info_new.addr2.data_buf中的数据写入到dma_data2.write_buf中
			spin_unlock(&dma_info_new.addr2_2.lock);
			copy_to_user((DMA_DATA_NEW*)arg, &dma_data2_2, sizeof(DMA_DATA_NEW));
			//spin_unlock(&dma_info_new.addr2.lock);
		break;

		case PCI_READ2_DATA3:
		        //printk("进入成功");
			spin_lock(&dma_info_new.flag_2.lock);
			//printk("发送成功1");
			memcpy(&idle_flag_2.flag_data, dma_info_new.flag_2.data_buf, 4);
			spin_unlock(&dma_info_new.flag_2.lock);
			//printk("发送成功2");
			copy_to_user((IDLE_FLAG*)arg, &idle_flag_2, sizeof(IDLE_FLAG));
			//spin_unlock(&dma_info_new.flag.lock);
			//printk("成功");
		break;
		
		case PCI_WRITE_TO_KERNEL_CMD:										
			spin_lock(&dma_info.addr_r.lock);//获取自旋锁
			copy_from_user(&dma_operation,(DMA_OPERATION*)arg, sizeof(DMA_OPERATION));//将用户空间的 DMA 操作结构体 arg 的数据复制到内核空间的 DMA 操作结构体 dma_operation 中
			memcpy(dma_info.addr_r.data_buf, dma_operation.data.write_buf, dma_operation.current_len*4);//将dma_operation中的数据写入到dma_info.addr_r.data_buf中
			memset(dma_info.addr_w.data_buf, 0, dma_operation.current_len*4);//将dma_info.addr_w.data_buf中的数据清零
		...
		case PCI_DMA_WRITE_CMD:												/* CPU读数据 */
			...
			memset(dma_info.addr_w.data_buf, 0, dma_operation.current_len*4);//将dma_info.addr_w.data_buf中的数据清零
			set_dma_w_r(dma_info.cmd.value, pci_pango); //可以理解为发送一个写命令给FPGA设备，FPGA设备接收到这个命令后，会根据命令的内容进行相应的操作
			set_dma_addr(&dma_info.addr_w, pci_pango);  //将DMA地址发送给FPGA设备，FPGA设备接收到这个地址后，会根据这个地址进行相应的操作
			spin_unlock(&dma_info.addr_r.lock);
		break;
		
		case PCI_READ_FROM_KERNEL_CMD:										/* 将数据写入CPU缓存 */
			spin_lock(&dma_info.addr_r.lock);
			memcpy(dma_operation.data.read_buf, dma_info.addr_w.data_buf, dma_operation.current_len*4);//此时，ed已经将数据写入到了dma_info.addr_w.data_buf中，现在将数据从dma_info.addr_w.data_buf中读取到dma_operation.data.read_buf中
			copy_to_user((DMA_OPERATION*)arg, &dma_operation, sizeof(DMA_OPERATION));//将 DMA 操作结构体 dma_operation（更新后的，里面包含了dma写的数据） 的数据复制到用户空间的 DMA 操作结构体 arg 中
			spin_unlock(&dma_info.addr_r.lock);
		break;
