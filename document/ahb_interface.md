...
选择的突发传输模式为环回，`burst_len=4`。在要传输的第一个地址位置，`HTRANS` 信号都要置为 `NONSEQ`，其后置为 `SEQ`，由于是突发传输，因此只需要给首地址即可。

在指示主设备往从设备写时，`HWRITE` 置为高指示写，同时要传输的每个数据单元数据位宽由 `HSIZE` 指定，`HWSTRB` 用于掩码要传输的字节，如果不作掩码，则 `HWSTRB={number_bytes{1'b1}}`。当从设备响应 `HREADY` 信号时，当前在 `HWDATA` 上的数据才可以被写入。
...
主设备读与写的区别就在于将 `HWRITE` 信号置为低。注意，`HRDATA` 在从设备发出准备好信号后的下一个时钟周期才有效。
