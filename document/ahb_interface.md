...
| 3'b111 | INCR16 | 递增模式， `burst_len=16` |
  ...
  * `HPROT[3:0]`：保护类型信号，指定传输的属性（如特权级别、缓存属性等）。一般可以设置为 `4'b0011`。
...
| 特点 | AHB3 | AHB2 |
| :--- | :--- | :--- |
| 地址宽度 | 通常为 32 位 | 通常为 32 位 |
| 数据宽度 | 支持 64 位、128 位 | 通常为 32 位 |
| 信号传输模型 | 支持低功耗（Low Power Interface）模式 | 不支持 |
| 突发传输支持 | 支持 | 支持 |
| 信号 | 增加了信号保护（Protection Signals） | 没有 | 
| 错误响应机制 | 增强了错误响应 | 错误响应较为简单 |
| 多层架构支持（Multilayer） | 支持（可以搭配 AXI 总线进行扩展） | 不支持 |
| 缓存和 QoS 支持 | 支持缓存控制和 QoS 信号 | 不支持 |